...
The benefit of this design is that the hash can be referenced in other data structures or stored locally and be used to verify that the server is not hiding data or sending old data. While it's not possible to prevent forks by the server it highly discourages due client side verification.

## Keys encryption strategy
...
The content to get signed should always be a string.
