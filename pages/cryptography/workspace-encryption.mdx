...
All keys (except document attachment keys) to encrypt data are derived from the currently active workspace key using key derivation. Currently these are keys to encrypt:
...
In the future this will include the workspace name and other workspace metadata.
...
- noncegen: `sodium.randombytes_buf(sodium.crypto_secretbox_NONCEBYTES)` // TODO this should not be crypto_secretbox_NONCEBYTES, but rather crypto_box_NONCEBYTES - which are probably the same
